...
describe("redeemInviteSchema", () => {
  ...
  it("enforces avatar payload limits without changing optional behavior", () => {
    expect(
      safeParse(redeemInviteSchema, {
        ...redeemInput,
        avatar: "a".repeat(MAX_AVATAR_DATA_URL_LENGTH),
      }).success,
    ).toBe(true)
    expect(
      safeParse(redeemInviteSchema, {
        ...redeemInput,
        avatar: "a".repeat(MAX_AVATAR_DATA_URL_LENGTH + 1),
      }).success,
    ).toBe(false)
    expect(
      safeParse(uploadAvatarSchema, {
        imageBase64: "a".repeat(MAX_AVATAR_BASE64_LENGTH + 1),
        mimeType: "image/png",
      }).success,
    ).toBe(false)
  })
...
describe("shared schema regressions", () => {
  it("preserves all password checks in their original order", () => {
    expect(
      schemaIssues(passwordSchema, "123").map((issue) => issue.message),
    ).toEqual([
      "validation.passwordMinLength",
      "validation.passwordUppercase",
      "validation.passwordLowercase",
    ])
    expect(schemaIssues(passwordSchema, "abcdefgh")).toEqual([
      { path: [], message: "validation.passwordUppercase" },
    ])
    expect(schemaIssues(passwordSchema, "ABCDEFGH")).toEqual([
      { path: [], message: "validation.passwordLowercase" },
    ])
    expect(parse(passwordSchema, "Abcdefgh")).toBe("Abcdefgh")
    expect(parse(passwordSchema, "Aa😀😀😀")).toBe("Aa😀😀😀")
    expect(parse(loginSchema, { username: " user ", password: " " })).toEqual({
      username: " user ",
      password: " ",
    })
  })
  ...
  it.each([
    ...
    [
      resetPasswordFormSchema,
      { pin: "123", newPassword: "short", confirmPassword: "other" },
    ],
    [
      inviteRedemptionFormSchema,
      {
    ...
    "retains confirmation errors alongside password constraint errors",
    (schema, value) => {
      expect(schemaIssues(schema, value)).toContainEqual({
  ...
  it.each([
    ...
    expect(schemaIssues(schema, { unknown: true })).toEqual([
      { path: [], message },
    ])
  })
  ...
  it("accepts explicit nulls and false values in patches", () => {
    expect(
      parse(updateManagedUserSchema, {
        email: null,
        expiresAt: null,
        isDisabled: false,
      }),
    ).toEqual({ email: null, expiresAt: null, isDisabled: false })
    expect(
      parse(updateInviteSchema, {
        useLimit: null,
        expiresAt: null,
        isDisabled: false,
      }),
    ).toEqual({ useLimit: null, expiresAt: null, isDisabled: false })
    expect(parse(updateMyAccountSchema, { email: null, locale: null })).toEqual(
      { email: null, locale: null },
    )
    expect(
      schemaIssues(updateManagedUserSchema, { email: "invalid" }),
    ).toContainEqual({ path: ["email"], message: "validation.invalidEmail" })
    expect(
      schemaIssues(updateManagedUserSchema, { profileId: "invalid" }),
    ).toContainEqual({
      path: ["profileId"],
      message: "validation.invalidProfileId",
    })
  })
  ...
  it("retains future-expiry checks for request strings and form Dates", () => {
    for (const schema of [
      ...
      expect(
        schemaIssues(schema, { profileId, expiresAt: "2000-01-01T00:00:00Z" }),
      ).toContainEqual({
        path: ["expiresAt"],
        message: "validation.expiryFuture",
      })
      expect(
        safeParse(schema, { profileId, expiresAt: "2099-01-01T00:00:00Z" })
          .success,
      ).toBe(true)
    }
    const value = {
    ...
    expect(
      safeParse(inviteFormSchema, {
        ...value,
        expiresAt: value.expiresAt.toISOString(),
      }).success,
    ).toBe(false)
    expect(
      safeParse(inviteFormSchema, { ...value, expiresAt: new Date("invalid") })
        .success,
    ).toBe(false)
  })
  ...
  it("retains optional invite code and numeric-limit differences between create/update", () => {
    ...
    expect(
      parse(inviteFormSchema, { profileId, code: "  ", expiresAt: null }),
    ).toEqual({ profileId, code: "", expiresAt: null })
    expect(
      schemaIssues(inviteFormSchema, {
        profileId,
        code: "short",
      ...
      path: ["code"],
      message: "validation.inviteCodeFormPattern",
    })
    expect(
      safeParse(createInviteSchema, { profileId, useLimit: 1.5 }).success,
    ).toBe(false)
    expect(
      safeParse(createInviteSchema, {
        profileId,
        useLimit: Number.MAX_SAFE_INTEGER + 1,
      }).success,
    ).toBe(false)
    expect(parse(updateInviteSchema, { useLimit: 1.5 })).toEqual({
      useLimit: 1.5,
    })
  })
  ...
  it("defaults nested profile permissions and validates renewal boundaries", () => {
    ...
    const input = { name: "Default", policy }
    expectTypeOf(input).toExtend<StaticEncode<typeof createProfileSchema>>()
    expectTypeOf<
      StaticDecode<typeof createProfileSchema>["policy"]["seerrPermissions"]
    >().toEqualTypeOf<number>()
    expect(parse(createProfileSchema, input).policy).toEqual({
      ...policy,
      seerrPermissions: DEFAULT_SEERR_PERMISSIONS,
    })
    expect(
      safeParse(createProfileSchema, {
        name: "Profile",
        policy: { ...policy, seerrPermissions: Number.MAX_SAFE_INTEGER + 1 },
      }).success,
    ).toBe(false)
    expect(
      safeParse(createProfileSchema, {
        name: "Default",
        policy: { ...policy, seerrPermissions: null },
      }).success,
    ).toBe(false)
    expect(
      parse(profileRenewalPolicySchema, {
        mode: "self-serve",
        extendByDays: 3650,
        maxTotalDays: 36500,
        minLeadTimeHours: 8760,
      }),
    ).toEqual({
      mode: "self-serve",
      extendByDays: 3650,
    ...
    expect(
      safeParse(profileRenewalPolicySchema, {
        mode: "disabled",
        extendByDays: 3651,
      }).success,
    ).toBe(false)
  ...
  it("validates quota limits and days only for enabled limited overrides", () => {
    ...
    expect(
      safeParse(profileFormSchema, { ...defaultFormValues, name: "Profile" })
        .success,
    ).toBe(true)
    expect(
      safeParse(profileFormSchema, {
        ...value,
        seerrMovieQuotaLimit: " 100 ",
        seerrMovieQuotaDays: "1",
        seerrTvQuotaLimit: "1",
        seerrTvQuotaDays: "100",
      }).success,
    ).toBe(true)
    expect(
      safeParse(profileFormSchema, {
        ...value,
        seerrMovieQuotaLimit: "101",
        seerrMovieQuotaDays: "0",
        seerrTvQuotaLimit: "01",
        seerrTvQuotaDays: "1.5",
      }).success,
    ).toBe(false)
  })
  ...
  it("keeps SMTP conditional requirements and existing parseInt port behavior", () => {
    ...
    expect(
      schemaIssues(emailSettingsFormSchema, { ...blank, from: "sender" }),
    ).toEqual([
      { path: ["smtpHost"], message: "validation.smtpHostRequired" },
      { path: ["smtpPort"], message: "validation.smtpPortRequired" },
    ])
    expect(
      schemaIssues(emailSettingsFormSchema, {
        ...blank,
        smtpHost: "smtp",
        smtpPort: "65536",
      }),
    ).toEqual([{ path: ["smtpPort"], message: "validation.smtpPortRange" }])
    expect(
      safeParse(emailSettingsFormSchema, {
        ...blank,
        smtpHost: "smtp",
        smtpPort: "25suffix",
      }).success,
  ...
  it("keeps optional Seerr setup and blank URL behavior", () => {
    expect(
      parse(onboardingSeerrFormSchema, {
        internalUrl: "",
        externalUrl: "",
        apiKey: "",
      }),
    ).toEqual({ internalUrl: "", externalUrl: "", apiKey: "" })
    expect(
      schemaIssues(onboardingSeerrFormSchema, {
        internalUrl: "",
    ...
    expect(
      schemaIssues(appSettingsFormSchema, {
        title: "App",
        description: "",
        defaultLocale: "en",
        url: "invalid",
      }),
    ).toContainEqual({ path: ["url"], message: "validation.validUrlRequired" })
  })

  it("accepts whitespace-only optional email without normalizing it", () => {
    expect(
      parse(optionalEmailAccountFormSchema, { name: "user", email: "  " }),
    ).toEqual({ name: "user", email: "  " })
    expect(
      schemaIssues(optionalEmailAccountFormSchema, {
        name: "user",
        email: " user@example.com ",
      }),
    ).toEqual([{ path: ["email"], message: "validation.invalidEmail" }])
  })

  it("trims onboarding pages before length checks and reports nested field paths", () => {
    expect(
      parse(memberOnboardingSettingsFormSchema, {
        enabled: true,
        pages: [{ id: "1", title: " Title ", markdown: " content " }],
      }),
    ).toEqual({
      enabled: true,
      pages: [{ id: "1", title: "Title", markdown: "content" }],
    ...
    expect(
      schemaIssues(memberOnboardingSettingsFormSchema, {
        enabled: true,
        pages: [{ id: "1", title: " ", markdown: " " }],
      }),
    ).toEqual([
      { path: ["pages", 0, "title"], message: "validation.pageTitleRequired" },
      {
        path: ["pages", 0, "markdown"],
        message: "validation.pageContentRequired",
      },
    ])
  })
})
